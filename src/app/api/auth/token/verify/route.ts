...
import type { CooldownStatus } from '@/types/auth'
 ...
 * @module IAM
 * @body { email: string }
 * @returns { message: string, cooldownSeconds: number }
 */
...
/**
 * @method GET /api/auth/token/verify?email=xxx
 * @namespace AuthToken
 * @description Check cooldown status for resending verification email
 * @module IAM
 * @query email - User email to check cooldown for
 * @returns CooldownStatus { cooldownActive: boolean, remainingSeconds: number }
 */
export async function GET(req: Request): Promise<NextResponse<CooldownStatus | { error: string }>> {
    try {
