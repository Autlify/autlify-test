...
export type PermissionEntitlementGate = {
   ...
   *
   * Standard (strict):
   * - Use EITHER SubModuleKey (module.submodule.) OR FeatureKey (module.submodule.resource.)
   * - Never go deeper than 3 segments in the prefix.
   */
...
export function getGateForPermissionKey(permissionKey: ActionKey): PermissionEntitlementGate | null {
...
export function isPermissionAssignable(
  ...
  entitlements: Partial<Record<FeatureKey, EffectiveEntitlement>>
): boolean {
