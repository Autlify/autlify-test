...
export function UsageTable({
      ...
      return columnsToExport
        .map((col) => {
          ...
          if (key === "model") {
            ...
            formattedValue = formatNumber(Number(value ?? 0));
          }
  ...
  }, [usageHistory, totalRow, showTotal, hasApiCost, hasCostToYou]);
  ...
  return (
    <div className="rounded-lg border border-border/50 bg-surface-secondary p-6">
      <Card className={cn("w-full shadow-lg", className)}>
        <CardHeader className="flex flex-row items-start justify-between space-y-0">
          ...
          {usageHistory.length > 0 && (
            <Button
              ...
              variant="outline"
              size="sm"
              className="ml-4 h-8 shadow-lg hover:shadow-xl transition-all duration-200"
            >
        ...
        <CardContent>
          <div className="overflow-x-auto rounded-md border border-border/30 backdrop-blur-sm">
            <Table>
