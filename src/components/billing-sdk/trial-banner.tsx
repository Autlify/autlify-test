...
export function TrialBanner({
  ...
  // Calculate end date from daysRemaining if trialEndDate not provided
  const effectiveEndDate = trialEndDate || (propDaysRemaining 
    ? new Date(Date.now() + propDaysRemaining * 24 * 60 * 60 * 1000)
    : null);
  
  const [timeRemaining, setTimeRemaining] = useState<TimeRemaining>(() => {
    if (effectiveEndDate) {
      return calculateTimeRemaining(effectiveEndDate);
    }
    return { days: 0, hours: 0, minutes: 0, seconds: 0 };
  });
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setTimeRemaining(calculateTimeRemaining(effectiveEndDate));
    }, 1000);
